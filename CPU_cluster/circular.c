...
ptr_t stencil_27(ptr_t grid, ptr_t aux, const dist_grid_info_t *grid_info, int nt) {
	...
	int cir = grid_info->halo_size_x;
	int t_sp = cir, y_sp = ly/32, z_sp = lz;
	double *a;
	double* p[30][20][3];
	int numy = ly/y_sp;
	int num_t = 24;
	a = ( double * )malloc( sizeof(double)*(t_sp-1)*ldx*(y_sp+2*cir)*3*num_t );
	for( int k = 0; k < num_t; k ++ )
        for( int i = 0; i < t_sp-1; i ++ ){
            for( int j = 0; j < 3; j ++ ){
                p[k][i+1][j] = &a[ (i*3+j)*ldx*(y_sp+2*cir)+k*(t_sp-1)*ldx*(y_sp+2*cir)*3 ];
            }
        }
	int nw = 0;
	omp_set_num_threads(num_t);
			...
			for( int yy = y_start; yy < y_end; yy += y_sp ){//进行y z两层block
				...
				for( int w = 1; w < t_sp; w ++ )
					for( int kk = 0; kk < 3; kk ++ )
						for( int j = 0; j < y_sp+cir*2; j ++ )
							for( int i = 0; i < ldx; i ++ )
								p[id][w][kk][INDEX(i, j, 0, ldx, 0)] = 0.0;
				for( int k = zz-cir; k < zz+z_sp+cir; k ++ ){
					int ss = min( step, (k-zz+cir)/2 );//ss 为要迭代的步数
					p[id][0][k%3] = &a0[INDEX(0, yy-cir, k, ldx, ldy)];
					if( ss <= 0 ) continue;
					if( ss == step )
						p[id][ss][(k-ss)%3] = &a1[INDEX(0, yy-cir, k-ss, ldx, ldy)];
					for( int s = 1; s <= ss; s ++ ){
						if( k-s < z_start || k-s >= z_end ){
                            for( int y = 0; y < y_sp+2*cir; y ++ )
                                for( int x = 0; x < lx+2*cir; x ++ )
                                    p[id][s][(k-s)%3][INDEX(x, y, 0, ldx, ldy)] = 0.0;
                            continue;
						}
						int y0, y1;
#ifdef __OUT
                        printf("y %d z %d step %d:\n", yy, k-s, s);
#endif
                        y0 = max(yy-cir+s,y_start)-yy+cir;
						y1 = min(yy+y_sp+cir-s, y_end)+cir-yy;
						for( int y = y0; y < y1; y ++ ){
							//for( int x = s; x < lx+2*cir-s; x ++ ){
							for( int x = cir; x < lx+cir; x ++ ){
								p[id][s][(k-s)%3][INDEX(x, y, 0, ldx, ldy)] 
								= ALPHA_ZZZ * p[id][s-1][ ( k-s )%3 ][INDEX(x, y, 0, ldx, ldy)] \
								+ ALPHA_NZZ * p[id][s-1][ ( k-s )%3 ][INDEX(x-1, y, 0, ldx, ldy)] \
								+ ALPHA_PZZ * p[id][s-1][ ( k-s )%3 ][INDEX(x+1, y, 0, ldx, ldy)] \
								+ ALPHA_ZNZ * p[id][s-1][ ( k-s )%3 ][INDEX(x, y-1, 0, ldx, ldy)] \
								+ ALPHA_ZPZ * p[id][s-1][ ( k-s )%3 ][INDEX(x, y+1, 0, ldx, ldy)] \
								+ ALPHA_ZZN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x, y, 0, ldx, ldy)] \
								+ ALPHA_ZZP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x, y, 0, ldx, ldy)] \
								+ ALPHA_NNZ * p[id][s-1][ ( k-s )%3 ][INDEX(x-1, y-1, 0, ldx, ldy)] \
								+ ALPHA_PNZ * p[id][s-1][ ( k-s )%3 ][INDEX(x+1, y-1, 0, ldx, ldy)] \
								+ ALPHA_NPZ * p[id][s-1][ ( k-s )%3 ][INDEX(x-1, y+1, 0, ldx, ldy)] \
								+ ALPHA_PPZ * p[id][s-1][ ( k-s )%3 ][INDEX(x+1, y+1, 0, ldx, ldy)] \
								+ ALPHA_NZN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x-1, y, 0, ldx, ldy)] \
								+ ALPHA_PZN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x+1, y, 0, ldx, ldy)] \
								+ ALPHA_NZP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x-1, y, 0, ldx, ldy)] \
								+ ALPHA_PZP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x+1, y, 0, ldx, ldy)] \
								+ ALPHA_ZNN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x, y-1, 0, ldx, ldy)] \
								+ ALPHA_ZPN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x, y+1, 0, ldx, ldy)] \
								+ ALPHA_ZNP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x, y-1, 0, ldx, ldy)] \
								+ ALPHA_ZPP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x, y+1, 0, ldx, ldy)] \
								+ ALPHA_NNN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x-1, y-1, 0, ldx, ldy)] \
								+ ALPHA_PNN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x+1, y-1, 0, ldx, ldy)] \
								+ ALPHA_NPN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x-1, y+1, 0, ldx, ldy)] \
								+ ALPHA_PPN * p[id][s-1][ ( k-s-1 )%3 ][INDEX(x+1, y+1, 0, ldx, ldy)] \
								+ ALPHA_NNP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x-1, y-1, 0, ldx, ldy)] \
								+ ALPHA_PNP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x+1, y-1, 0, ldx, ldy)] \
								+ ALPHA_NPP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x-1, y+1, 0, ldx, ldy)] \
								+ ALPHA_PPP * p[id][s-1][ ( k-s+1 )%3 ][INDEX(x+1, y+1, 0, ldx, ldy)];								
                    ...
                    for( int y = 0; y < y_sp+2*cir; y ++ ){
                        for( int x = 0; x < lx+2*cir; x ++ ){
                            printf("%10f ", p[id][s][(k-s)%3][INDEX(x, y, 0, ldx, ldy)]);
                        }
                        puts("");
                    }
                    puts("");
                    ...
                    for( int y = 0; y < y_sp+2*cir; y ++ ){
                        for( int x = 0; x < lx+2*cir; x ++ ){
                            printf("%10f ", p[id][s-1][(k-s)%3][INDEX(x, y, 0, ldx, ldy)]);
                        }
                        puts("");
                    }
                    puts("");
